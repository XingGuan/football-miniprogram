...
Page({
    ...
    try {
      ...
      const rankList = Object.values(userStats)
        ...
        .map((u) => ({
          ...
          winRate:
            u.totalRecords > 0
              ? Math.round((u.winRecords / u.totalRecords) * 100)
              : 0,
        }))
        .sort((a, b) => {
          // 先按胜率排序，胜率相同按中奖数排序
  ...
  async loadUserMedals(userStats) {
    ...
    const medalPromises = userIds.map(async (userId) => {
      try {
        ...
        const medalList = Array.isArray(medals)
          ? medals
          : (medals && medals.data) || [];
        ...
        const acquiredMedals = medalList
          .filter(
            (m) =>
              m.acquireTime && m.acquireTime !== "null" && m.acquireTime !== "",
          )
          ...
          for (const combo of matchCombinations) {
            ...
            for (const matchId of combo) {
              ...
              const selectedOptions =
                (matchPlayTypes[matchId] || {})[playType] || [];
              const hitSelected = selectedOptions.find(
                (opt) => opt.isHit === true,
              );
              if (!hitSelected) {
                allHit = false;
  ...
  async loadLeagues() {
    ...
    try {
      ...
      const activeLeagues = (leagues || [])
        ...
        .map((item) => ({
          ...item,
          firstChar: (item.leagueAbbrCnName || item.leagueName || "").substring(
            0,
            1,
          ),
        }));
  ...
  async onSelectLeague(e) {
    ...
    try {
      ...
      if (sortedSeasons.length > 0) {
        this.onSelectSeason({
          currentTarget: { dataset: { season: sortedSeasons[0] } },
        });
      }
    } catch (e) {
  ...
  async onSelectSeason(e) {
    ...
    try {
      const leagueApi = require("../../api/league");
      const standings = await leagueApi.getStanding(
        selectedLeague.id,
        season.id,
      );
      // 缓存所有数据
      const allStandings = (standings || []).filter(
        (item) => item.status === 1,
      );
      // 过滤出当前排名类型的数据
      const filteredStandings = this.filterStandings(
        allStandings,
        standingType,
      );
